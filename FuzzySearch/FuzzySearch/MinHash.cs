...
public class MinHash
{
    ...
    public List<int> getTokenID(string s)
    {
        ...
        for (int i = 0; i < arr.Length - k + 1; i++)
        {
            ...
            if (temp >= 0)
                tockens.Add(temp % 8000);
            else
                tockens.Add(temp % 8000 + 8000);
        }
        return tockens;
    }

    public List<int> getTokenID(int vector)
    {
        List<int> tockens = new List<int>();

        int temp = vector.GetHashCode();
        if (temp >= 0)
    ...
    public uint[] getMinHashSignatures(int[] vectors)
    {
        uint[] minHashes = new uint[numHashFunctions];
